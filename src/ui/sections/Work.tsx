...
/** Panel order follows the CV: professional experience first, then projects. */
const GROUPS: Array<[ProjectKind, string]> = [
  ...
  return (
    <div className="sec">
      {GROUPS.map(([kind, heading]) => (
        <Fragment key={kind}>
          <h3 className="sec-sub">{heading}</h3>
          <ol className="rows">
            {projects.map((project, i) => {
              if (project.kind !== kind) return null
              const open = i === active
              return (
                <li
                  key={project.id}
